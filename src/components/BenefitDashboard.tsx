...
export type DashboardMode = 'consumable' | 'ongoing';

export interface BenefitDashboardProps {
  mode: DashboardMode;
  title: string;
  subtitle?: string;
...
/**
 * Reusable dashboard body.
 *
 * `consumable` mode: shows only benefits that are consumed over time (annual,
 * semi_annual, quarterly, monthly, one_time, spend_threshold). Renders progress
 * bars, "used / remaining" counts, and a Log Usage button.
 *
 * `ongoing` mode: shows only unlimited/perpetual benefits (earning multipliers,
 * no-FTF, complimentary status perks, insurance). Renders as reference cards
 * with no usage tracking.
 */
export default function BenefitDashboard({ mode, title, subtitle, emptyMessage }: BenefitDashboardProps) {
  const now = new Date();
  ...
  const filtered = useMemo(() => {
    return modeFiltered.filter(p => {
      ...
      if (mode === 'consumable') {
        if (cadence === 'other') {
          if (['annual','semiannual','quarterly','monthly'].includes(p.benefit.reset_cadence)) return false;
        } else if (cadence !== 'all') {
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {isConsumable && (
          <div className="flex gap-1 rounded-lg border border-slate-200 dark:border-slate-800 p-1">
            {[refYear - 1, refYear, refYear + 1].map((_, i) => {
...
function ConsumableTile({ p, onLogUsage, onChanged }: { p: BenefitProjection; onLogUsage: () => void; onChanged: () => void }) {
  ...
  async function toggleUsed() {
    ...
    try {
      ...
      // Surface via alert — tile has no dedicated error slot.
      alert(`Failed to update usage: ${e}`);
    } finally {
      setBusy(false);
    }
  }
...
function OngoingTile({ p }: { p: BenefitProjection }) {
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-800 rounded-lg p-3 flex flex-col gap-2 bg-slate-50 dark:bg-slate-900/40">
      <div className="flex items-start gap-2">
        <span className="mt-1.5 inline-block w-2 h-2 rounded-full bg-primary-500" title="ongoing" />
        <div className="flex-1 min-w-0">
          <div className="font-medium leading-tight break-words">{b.title}</div>
          <div className="text-xs text-slate-500 mt-0.5">
            Ongoing · {categoryLabel(b.category)}
          </div>
        </div>
      </div>
      ...
      {b.notes && (
        <div className="text-xs text-slate-500 italic leading-snug">
          {b.notes}
        </div>
      )}
