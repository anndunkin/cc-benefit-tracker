...
### Changed
- **Bonus Categories tab.** New page (`src/pages/BonusCategories.tsx`, route `/bonus-categories`) groups every `earning_multiplier` benefit across all 11 cards by spend category (airfare, hotels, car rentals, dining, groceries, gas, rideshare/transit, streaming/entertainment, online shopping, everyday/everything else), sorted by points multiplier descending within each bucket, so it's easy to see which card to use for a given purchase. Statement-credit benefits are intentionally excluded — this tab is earning-multipliers only.
  - Added 4 new structured fields already present on `Benefit`/`BenefitInput` (`multiplier_rate`, `multiplier_currency`, `spend_category`, `spend_category_note`) to every `earning_multiplier` row for all 11 cards in `electron/benefitsSeedData.ts` (34 new rows), and backfilled the 6 pre-existing `marriott_premier` and `virgin_atlantic` earning-multiplier rows with the same structured fields.
  - Rates verified against official issuer pages: [Amex Platinum](https://www.americanexpress.com/us/credit-cards/card/platinum/), [Delta Reserve](https://www.americanexpress.com/us/credit-cards/card/delta-skymiles-reserve-american-express-card/), [Hilton Aspire](https://www.americanexpress.com/us/credit-cards/card/hilton-honors-aspire/), [Citi AAdvantage Executive](https://www.citi.com/credit-cards/citi-aadvantage-executive-credit-card), [Marriott Premier](https://marriott.chase.com/premier), [Marriott Bonvoy Business](https://www.marriott.com/credit-cards/marriott-bonvoy-business-american-express-card.mi), [World of Hyatt](https://creditcards.chase.com/travel-credit-cards/world-of-hyatt), [British Airways Visa](https://creditcards.chase.com/travel-credit-cards/avios/british-airways), [Virgin Atlantic (Synchrony)](https://www.synchrony.com/partner/virgin-red-rewards-card), [IHG Premier](https://www.chase.com/personal/credit-cards/ihg/premier/earn-rewards), and [Citi Prestige (Frequent Miler)](https://frequentmiler.com/typrestige/) for the closed-to-new-applicants card with no live Citi product page.
  - **Reconciled conflicts using the most authoritative source:** IHG Premier's IHG-hotel earning rate is recorded as the 10x card-level rate straight from Chase's own breakdown page (which separately documents the additional up-to-16x from IHG One Rewards membership + card-granted Platinum Elite status, for an "up to 26x" total that isn't a pure card-spend multiplier). Marriott Bonvoy Business's secondary tier is recorded as 4x (dining worldwide / U.S. gas / U.S. shipping / U.S. wireless) per the official U.S. issuer page (`marriott.com/credit-cards/marriott-bonvoy-business-american-express-card.mi`, mirrored on `americanexpress.com/us/...`), superseding the 3x Canadian-card figure that had been carried in the initial research notes.
- **Points Currency Values tab.** New page (`src/pages/PointsCurrencies.tsx`, route `/points-currencies`) lists every `PointsCurrency` row grouped by type (transferable / airline / hotel), each showing name, value formatted as `¢X.X per point`, a source link, and an inline edit modal (mirroring `BenefitEditor.tsx`'s create/edit pattern) that sets `is_user_modified` so quarterly refreshes never silently overwrite a manual override.
  - Seeded the 10 currencies relevant to the tracked 11 cards + 4 programs with August 2026 figures from [One Mile at a Time](https://onemileatatime.com/guides/value-miles-points/) (Ben Schlappig): Amex Membership Rewards ¢1.7, Citi ThankYou ¢1.7, Delta SkyMiles ¢1.1, American AAdvantage ¢1.5, British Airways Avios ¢1.3, Virgin Atlantic Flying Club ¢1.1, Marriott Bonvoy ¢0.7, Hilton Honors ¢0.5, World of Hyatt ¢1.5, IHG One Rewards ¢0.5.
  - Added full CRUD (`pointsCurrenciesGetAll/GetById/Create/Update/Delete` in `electron/database.ts`, mirroring the `programCreate`/`programUpdate` COALESCE partial-update + slugify-id pattern), IPC handlers (`pointsCurrencies:*` in `electron/main.ts`), and the `preload.ts` bridge.
  - Wired `points_currencies` into `AppFilePayload` export/import (`buildFilePayload`/`importFilePayload`) so Save/Export/Import round-trip the new table.
  - Extended the quarterly Refresh diff/review mechanism (`RefreshChange`, `refreshStartRun`/`refreshApplyRun` in `electron/database.ts`) with an optional `points_currency_id` column so future refreshes can propose updated point values through the same approve/reject workflow used for benefits — `is_user_modified` currencies are always skipped on apply, exactly like user-edited benefits.
- **Base program benefits (Feature 3).** Added a new `marriott_bonvoy_base` program (`program_type: 'other'`), distinct from the existing elite-status `marriott_status` program, holding the "Stay for 5, Pay for 4" award-stay benefit (5th night free on a 5+ consecutive-night, 100%-points redemption) available to every Marriott Bonvoy member regardless of elite tier or card ownership. Source: [Marriott Loyalty Program Terms and Conditions, section 3.3.d](https://www.marriott.com/loyalty/terms/default.mi).
  - Checked Marriott Bonvoy, World of Hyatt, IHG One Rewards, Hilton Honors, Best Western, Wyndham, and Choice. Only Marriott qualifies as a true no-status/no-card base-member benefit — the others were researched and explicitly excluded with source citations in `electron/benefitsSeedData.ts`: World of Hyatt has no equivalent perk; IHG's "4th night free" is a [Chase co-branded card benefit](https://www.ihg.com/content/us/en/customer-care/frequently-asked-questions), not a base One Rewards benefit; Hilton's "5th night free" requires [Silver elite status or above](https://www.hilton.com/en/hilton-honors/terms/); Best Western/Wyndham/Choice have no comparable base-member award-stay perk in their published terms.

### Fixed
- `BEN_COLS` in `electron/database.ts` was missing the 4 structured earning-multiplier columns (`multiplier_rate`, `multiplier_currency`, `spend_category`, `spend_category_note`), so every benefit SELECT query (`benefitsGetAll`, `benefitsForCard`, `benefitsForProgram`, `benefitGetById`, `computeProjections`) silently returned `undefined` for these fields even though the columns existed in the schema. Fixed by adding the 4 columns to `BEN_COLS`.
- `insertBenefit` (used by `seedIfFresh`), `benefitCreate`, and `benefitUpdate` did not accept or persist the 4 new columns — a benefit created or edited through the UI would silently drop any structured multiplier data. Both now read/write all 4 fields, including a direct-overwrite pattern on update (consistent with how `expiration_note`/`notes`/`source_url` are handled) so clearing a field back to `null` works as expected.
- `importFilePayload`'s benefit INSERT was missing `expiration_date`, `reset_years`, `multiplier_rate`, `multiplier_currency`, `spend_category`, `spend_category_note`, and `prerequisite_benefit_id` — a JSON import silently dropped these columns even before this release. Fixed alongside the v1.0.16 column work since it shares the same INSERT statement.

### Added
- v1.0.16 migration block in `applyDataMigrations()` (`electron/database.ts`), gated by `seedVersionLt(database, '1.0.16')`:
  - `ALTER TABLE benefits ADD COLUMN` for each of the 4 new columns on upgrading databases, checking `PRAGMA table_info(benefits)` first (and wrapping each `ALTER` in try/catch) so it's safe to run against both legacy pre-1.0.16 databases and hand-rolled test schemas.
  - Defensively `CREATE TABLE IF NOT EXISTS` for `refresh_runs`, `refresh_changes`, and `points_currencies` before altering/seeding them, so the migration is self-contained even against databases that predate those tables entirely.
  - `ALTER TABLE refresh_changes ADD COLUMN points_currency_id` for the Refresh-flow wiring described above.
  - Inserts every `SEED_POINTS_CURRENCIES` row that isn't already present by id, so upgrading (not just fresh) installs get the new reference table populated — rows a user has already created/edited with a matching id are left untouched.
- `src/lib/format.ts`: `spendCategoryLabel`, `currencyTypeLabel`, `fmtCentsPerPoint`, and `ALL_SPEND_CATEGORIES` helpers for the two new pages.
- `App.tsx` NAV array and `<Routes>` updated with the two new pages ("Bonus Categories" → `/bonus-categories`, "Points Values" → `/points-currencies`).

### Testing
- 157/157 Vitest tests pass (up from 139), across security, validation, boundary, and functionality suites.
- +18 new tests:
  - **Points currency validation** (5): rejects missing name, enforces `currency_type` CHECK, rejects non-numeric value, throws on updating a non-existent id, confirms all 10 seeded currencies land with correct type/value/source.
  - **Points currency CRUD lifecycle** (3): full create → update → delete round trip; `is_user_modified` is set on edit and never silently reset by a reseed; handles 50+ currencies without error.
  - **Points currency SQL injection resistance** (2): malicious name stored as literal data; malicious id-lookup string returns `null` instead of matching everything.
  - **Structured earning-multiplier column round-trip** (4): create/read/update round-trips for `multiplier_rate`/`multiplier_currency`/`spend_category`/`spend_category_note`; non-multiplier benefits keep these fields `null`; every one of the 11 seeded cards has at least one structured `earning_multiplier` row.
  - **Bonus Categories grouping logic** (2): buckets sort by `multiplier_rate` descending; no non-`earning_multiplier` benefit ever carries a `spend_category`.
  - **Base program benefit seeding** (2): `marriott_bonvoy_base` is a distinct program from `marriott_status`; the Stay-for-5-Pay-for-4 benefit seeds under it with `unlimited` cadence and an official `marriott.com` source.
- Updated 13 pre-existing migration-chain tests whose hardcoded `seed_version` expectation (`'1.0.14'`) was superseded by this release's `'1.0.16'` stamp, and the program-count assertion in the seed-data test (4 → 5, for the new `marriott_bonvoy_base` program).
- Client and Electron TypeScript checks (`tsc -p tsconfig.json`, `tsc -p tsconfig.node.json`) pass with zero errors.
- `npm run build` and `npm run electron:build` pass; see Notes for the signed artifact location.

### Notes
- All earning-rate and points-valuation figures are current as of August 2026 and cite their source directly in code comments in `electron/benefitsSeedData.ts` (per-row `source_url` fields) as well as above.
- Existing (pre-1.0.16) installs upgrade in place: the migration block back-fills the 4 new benefit columns and the `points_currencies` table without touching any existing benefit, usage, or user-modified data. New seed content (structured multiplier rows, the `marriott_bonvoy_base` program, the 10 points currencies) reaches existing installs the same way all prior seed refreshes have — through the in-app Quarterly Refresh review flow — while brand-new installs get everything immediately via `seedIfFresh`.
